...
for x in range(ratings_df_sample.shape[0]):
    lst[tuple(ratings_df_sample['userId'])[x]-1][tuple(ratings_df_sample['movieId'])[x]-1] = tuple(ratings_df_sample['rating'])[x]
...
lst_sim = np.array([[0.0]*len(ratings_df_sample['userId'].unique())]*len(ratings_df_sample['userId'].unique()))

for i in range(len(ratings_df_sample['userId'].unique())):
    for j in range(len(ratings_df_sample['userId'].unique())):
        if i != j:
